...
5. **使用 MySQL 存数据（可选）**  
   在 Zeabur 同一项目中添加 **MySQL** 服务，创建数据库与用户后，在 **writing-book** 服务的 Variables 中绑定或填写：
   - `MYSQL_HOST=mysql.zeabur.internal`（内网主机，Zeabur 会按服务名解析）
   - `MYSQL_PORT=3306`
   - `MYSQL_USER` / `MYSQL_PASSWORD` / `MYSQL_DATABASE`（与 MySQL 服务中一致）  
   也可使用 `DATABASE_URL=mysql://user:password@mysql.zeabur.internal:3306/database`。  
   配置后，用户、验证码、项目数据会写入 MySQL；未配置时使用 JSON 文件存储（无持久化卷时重启会丢失）。
...
- **MySQL**（可选）：配置 `MYSQL_HOST`、`MYSQL_PORT`、`MYSQL_USER`、`MYSQL_PASSWORD`、`MYSQL_DATABASE` 或 `DATABASE_URL` 后，用户与项目数据存入 MySQL；未配置时使用 `server/data/*.json` 文件（见 `.env.example`）。
