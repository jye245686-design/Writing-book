...
4. **配置环境变量**  
   ...
   - （可选）发送登录验证码短信：`TENCENT_SECRET_ID`、`TENCENT_SECRET_KEY`、`TENCENT_SMS_SDK_APP_ID`、`TENCENT_SMS_SIGN_NAME`、`TENCENT_SMS_TEMPLATE_ID`（见 1.3 节）
   - 无需设置 `PORT`，Zeabur 会自动注入。

5. **使用 MySQL 存数据（可选）**  
   - **优先：国内云数据库**（腾讯云、阿里云、华为云等）：  
     1. 在 [腾讯云 TencentDB](https://console.cloud.tencent.com/cdb)、[阿里云 RDS](https://rdsnext.console.aliyun.com/) 或 [华为云 RDS](https://console.huaweicloud.com/rds/) 开通 MySQL 实例（按量或包年均可）。  
     2. 在控制台查看**连接地址**（内网或公网）、**端口**（通常 3306）、**账号**、**密码**，并创建数据库（如 `writing_book`）。  
     3. 在 Zeabur 的 writing-book 服务 **Variables** 中填写：  
        `MYSQL_HOST=实例地址`、`MYSQL_PORT=3306`、`MYSQL_USER=账号`、`MYSQL_PASSWORD=密码`、`MYSQL_DATABASE=writing_book`。  
     应用启动时会自动建表。若应用与数据库在同一云厂商且同地域，建议用**内网地址**以省流量、降延迟。  
   - **或 Zeabur 同项目 MySQL**：在 Zeabur 同一项目中添加 MySQL 服务后，在 writing-book 的 Variables 中填写  
     `MYSQL_HOST=mysql.zeabur.internal`、`MYSQL_PORT=3306`、`MYSQL_USER`、`MYSQL_PASSWORD`、`MYSQL_DATABASE`，或使用 `DATABASE_URL=mysql://...`。  
   - **海外可选**：PlanetScale 等，在 Variables 中配置 `DATABASE_URL=mysql://...` 即可。  
   未配置任何 MySQL 时，使用 JSON 文件存储（无持久化卷时重启会丢失）。
...
### 线上请求很慢（10～20 秒才返回）怎么办？

本地很快、上线后首几次请求要等 10～20 秒，多半是**实例冷启动**或**数据库跨地域**导致。

| 原因 | 说明 | 建议 |
|------|------|------|
| **冷启动** | 一段时间无人访问后，平台会把实例休眠以省资源；下次有人访问时先“唤醒”再处理请求，首请求会等很久（常见 10～20 秒）。 | ① 在 Zeabur 中把该服务的**最小实例数**设为 1，避免缩容到 0；或 ② 用定时任务每 5 分钟访问一次 `https://你的域名/api/health` 做“保活”，减少冷启动。 |
| **MySQL 跨地域/公网** | 应用与数据库不在同一地域或走公网时，建连和查询延迟会明显增加。 | 应用与 MySQL 尽量**同地域**；同一云厂商下优先用**内网地址**（如腾讯云/阿里云 RDS 内网地址），并在 Variables 里填内网 host。 |
| **首请求建连** | 首次请求时建立 DB 连接池、建表等会多花几秒。 | 应用启动时已执行 `getPool()` 与建表，一般只影响“容器刚起来”的那一两秒；若仍慢，结合上面两点排查。 |

**保活示例（任选一种）：**

- **Uptime Robot / 监控服务**：新建 HTTP(s) 监控，地址填 `https://你的域名/api/health`，间隔 5 分钟。
- **自建 cron**：`*/5 * * * * curl -s -o /dev/null https://你的域名/api/health`

保活后，正常访问时实例已热启，列表、登录等接口会恢复到几百毫秒级。
...
- **MySQL**（可选）：优先推荐**国内云数据库**（腾讯云 / 阿里云 / 华为云 MySQL），配置 `MYSQL_HOST`、`MYSQL_PORT`、`MYSQL_USER`、`MYSQL_PASSWORD`、`MYSQL_DATABASE` 或 `DATABASE_URL` 后，用户与项目数据存入 MySQL；未配置时使用 `server/data/*.json` 文件（见 `.env.example`）。

`.env` 已列入 `.gitignore`，不要将含真实 Key 的 `.env` 推送到仓库。

### 1.2 登录方式

当前为**用户 ID + 密码**注册与登录（用户自定义 4～24 位 ID、至少 6 位密码）。**短信登录**（手机号 + 验证码）接口已保留（`/api/auth/send-code`、`/api/auth/login` 传 `phone`/`code`），待短信服务审核通过后可在前端开放；届时在 Zeabur/环境变量中配置腾讯云短信（见下方 1.3）即可发送真实验证码。

### 1.3 短信服务（腾讯云，可选）

若需发送真实验证码短信，在部署平台配置以下环境变量（需先在 [腾讯云短信控制台](https://console.cloud.tencent.com/smsv2) 开通短信服务、创建应用、申请国内短信签名与正文模板，验证码模板占位为 `{1}`）：

- **TENCENT_SECRET_ID** / **TENCENT_SECRET_KEY**：在 [API 密钥管理](https://console.cloud.tencent.com/cam/capi) 创建或使用已有密钥
- **TENCENT_SMS_SDK_APP_ID**：短信控制台「应用管理」中创建应用后得到的 SdkAppId（如 1400006666）
- **TENCENT_SMS_SIGN_NAME**：国内短信签名内容（已审核通过的签名文字）
- **TENCENT_SMS_TEMPLATE_ID**：正文模板 ID（验证码类模板，如「您的验证码是{1}，5分钟内有效」则传一个参数即验证码）

未配置上述五项时，不会调用腾讯云接口，开发环境仍会在控制台打印验证码。

### 1.4 国内云数据库（可选，推荐）

优先使用**国内云厂商的 MySQL**，与 Zeabur/自建应用同地域时可用内网地址，延迟低、合规方便。

| 厂商 | 产品 | 控制台 / 文档 |
|------|------|----------------|
| **腾讯云** | 云数据库 MySQL（TencentDB） | [控制台](https://console.cloud.tencent.com/cdb) — 新建实例 → 获取内网/公网地址、端口、账号、密码 → 创建数据库 `writing_book` |
| **阿里云** | 云数据库 RDS MySQL | [控制台](https://rdsnext.console.aliyun.com/) — 创建实例 → 申请外网地址（或同 VPC 内网）→ 创建账号与数据库 |
| **华为云** | 云数据库 RDS MySQL | [控制台](https://console.huaweicloud.com/rds/) — 购买实例 → 查看连接信息 → 创建数据库 |

在应用所在环境（如 Zeabur Variables 或本机 `.env`）配置 **MYSQL_HOST**（实例连接地址）、**MYSQL_PORT**（通常 3306）、**MYSQL_USER**、**MYSQL_PASSWORD**、**MYSQL_DATABASE=writing_book**，或拼成 **DATABASE_URL=mysql://用户:密码@主机:3306/writing_book**。应用启动时会自动建表，无需在控制台执行建表 SQL。
