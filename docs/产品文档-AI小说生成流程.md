...
| 项目     | 说明 |
|----------|------|
| 输入     | 用户选择：**世界背景**（古代/现代/末世/星际）及可选**细分方向**；**题材**（玄幻/言情/悬疑/都市/科幻/武侠）；可选**核心创意**；可选**文本设定标签**（见下表）。 |
| 输出     | 形成「创作设定」对象（含 worldBackground、worldBackgroundSub、genre、coreIdea、optionalTags），供后续书名、角色、大纲、正文生成使用。 |
| 交互     | **已落实**：四类世界背景 + 细分方向按钮 + 最搭题材提示；**可选标签**分 7 层展示，每层含「建议补充」标签与「适配原因」说明，多选后写入 optionalTags，用于增强生成效果。 |
| 可选标签（7 层） | **机制/外挂**：系统、面板/词条、模拟器、签到、抽卡、天赋树、职业面板（目标清晰、升级快）。**时空/身份**：重生、穿越、穿书、平行世界、回档、夺舍（开局即冲突）。**结构玩法**：无限流、副本、规则怪谈、逃生、推演、多线叙事（副本即小高潮）。**内容风格**：轻克苏鲁、诡异复苏、国风民俗、赛博、轻沙雕、轻迪化（差异化+爽感）。**生活向爽点**：种田、基建、经营、养成、直播、开店（日常即升级）。**情感向钩子**：先婚后爱、追妻火葬场、双强、救赎、甜宠、带崽（情绪价值）。**目标导向**：复仇、逆袭、保命、护家人、冲榜封神、苟到无敌（主线明确）。 |
...
用户功能按阶段实施，项目数据存储为**数据库**（阶段 2 完成项目与用户的绑定与迁移）。**当前实现**：登录方式为**用户 ID + 密码**（注册与登录）；手机号 + 验证码接口已保留，待短信服务审核通过后可启用。
